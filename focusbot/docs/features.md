...
7. /app/planner/page.tsx（フォーム＆結果）

- ユーザーが学習目標や背景を入力
- 「送信」ボタンでAIにプロンプトを送信
- 生成された学習プランを画面に表示

useStateで3つの状態を分けている理由
| ステート      | 目的              |
| --------- | --------------- |
| `prompt`  | ユーザー入力内容の管理     |
| `result`  | AIが返してくる学習プラン   |
| `loading` | 読み込み中表示制御（UX向上） |
| `error`   | エラーメッセージ表示      |

=> ユーザー入力 → AI応答 → 結果表示 の基本ループが完成
http://localhost:3000/plannerにて画面の確認ができる

8. plannerフォームの入力バリデーションを追加

入力チェック（バリデーション）**をフロントエンド側で行います。
- 空文字の送信防止（例：「空のまま送信」は不可）
- 最小文字数：10文字以上（具体的な意図を持たせる）
- 最大文字数：300文字未満（冗長なプロンプトを防止）
